...
from turbo_d.quantization import TurboDCodec, scalar_quantize


def test_codec_round_trip_shape() -> None:
    rng = np.random.default_rng(0)
    vectors = rng.normal(size=(2, 16, 32)).astype(np.float32)
    codec = TurboDCodec(dim=32, bits=4, qjl_bits=32, seed=1, codebook_samples=20_000)
...
def test_more_bits_reduce_reference_error() -> None:
    ...
    codec_3 = TurboDCodec(dim=64, bits=3, seed=2, codebook_samples=20_000)
    codec_5 = TurboDCodec(dim=64, bits=5, seed=2, codebook_samples=20_000)
...
def test_dot_estimate_shape() -> None:
    ...
    codec = TurboDCodec(dim=32, bits=4, qjl_bits=64, seed=3, codebook_samples=20_000)
